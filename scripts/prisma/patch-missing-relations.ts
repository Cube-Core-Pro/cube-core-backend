// scripts/prisma/patch-missing-relations.ts
// Ejecuta: npx tsx scripts/prisma/patch-missing-relations.ts
...
type InsertSpec = { model: string; fieldLine: string };
...
function backup(src: string, dst: string) {
  if (!fs.existsSync(dst)) fs.copyFileSync(src, dst);
}

function findModelBlock(content: string, modelName: string): { start: number; end: number } | null {
...
function ensureField(block: string, fieldLine: string): { block: string; changed: boolean } {
