...
files.forEach(filePath => {
  if (fs.existsSync(filePath)) {
    ...
    const oldPattern = /const messages = errorMessages\[lang\] \|\| errorMessages\['en'\] \|\| errorMessages\.en;/g;
    const newPattern = `const messages = errorMessages[lang] || errorMessages.en;`;
    
    if (content.includes('const messages = errorMessages[lang] || errorMessages[\'en\'] || errorMessages.en;')) {
      content = content.replace(oldPattern, newPattern);
      
      // Also add non-null assertion for strict mode
      ...
      console.log(`Fixed strict messages in ${filePath}`);
      fs.writeFileSync(filePath, content);
    }
  }
});

console.log('Done fixing strict messages issues');